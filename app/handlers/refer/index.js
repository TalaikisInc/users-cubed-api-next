...
const tokenResponse = async (tokenData, userData, refEmail, final) => {
  const refToken = await generateToken(tokenData.email).catch((e) => final({ s: 400, e }))
  if (refToken) {
    userData.referred.push(refToken)
    userData.updatedAt = Date.now()
    const referringUser = `${userData.firstName} ${userData.lastName} <${tokenData.email}>`
    const sent = await sendReferEmail(refEmail, refToken, referringUser).catch((e) => final({ s: 400, e }))
    if (sent) {
      finalizeRequest('users', tokenData.email, 'update', userData, final)
    } else {
      final({ s: 400, e: t('error.refer_email') })
    }
  } else {
    final({ s: 400, e: t('error.id') })
  }
}
...
export const refer = async (data, final) => {
  ...
  if (valid) {
    ...
    const token = await tokenHeader(data).catch(() => final({ s: 403, e: t('error.wrong_token') }))
    const tokenData = await db.read('tokens', token).catch(() => final({ s: 403, e: t('error.cannot_read') }))
...
export const use = async (data, final) => {
  ...
  if (valid) {
    ...
    if (token) {
      const refData = await db.read('refers', token).catch(() => final({ s: 403, e: t('error.token_notfound') }))
      refData.used = true
...
export const register = async (data, final) => {
  ...
  if (valid) {
    ...
    if (token && email) {
      const userData = await db.read('users', email).catch(() => final({ s: 400, e: t('error.no_user') }))
      const tokenData = await db.read('refers', token).catch(() => final({ s: 400, e: t('error.token_notfound') }))
      if (!userData.referred.includes(token)) {
        userData.referred.push(token)
        userData.updatedAt = Date.now()
        await db.update('users', email, userData).catch(() => final({ s: 500, e: t('error.cannot_update') }))
