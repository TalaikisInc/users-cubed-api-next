...
import { create, read, update } from '../../lib/db'
...
const _generateToken = async (email, done) => {
  ...
  await create('refers', token, obj).catch(() => done(t('error.save')))
  done(null, token)
}
...
export const refer = async (data, done) => {
  ...
  if (valid) {
    ...
    const token = await tokenHeader(data)
      .catch(async () => await sendError(403, t('error.wrong_token'), done))
    const tokenData = await read('tokens', token)
      .catch(async () => await sendError(403, t('error.cannot_read'), done))
    ...
    if (tokenData.email && validRef && data.payload.to !== tokenData.email) {
      const userData = await read('users', tokenData.email)
        .catch(async () => await sendError(400, t('error.cannot_read'), done))
      if (userData) {
  ...
  if (valid) {
    ...
    if (token) {
      const refData = await read('refers', token)
        .catch(async () => await sendError(403, t('error.token_notfound'), done))
      refData.used = true
  ...
  if (valid) {
    ...
    if (token && email) {
      const userData = await read('users', email)
        .catch(async () => await sendError(400, t('error.no_user'), done))
      const tokenData = await read('refers', token)
        .catch(async () => await sendError(400, t('error.token_notfound'), done))
      if (!userData.referred.includes(token)) {
        userData.referred.push(token)
        userData.updatedAt = Date.now()
        await update('users', email, userData)
          .catch(async () => await sendError(500, t('error.cannot_update'), done))
