...
import { user, countries, validUser } from '../../lib/schemas'
import { COMPANY, FRONTEND_URL, ROLES, FIRST_CONFIRM, USER_TOKEN_EXPIRY } from '../../config'
...
const _sendEmailConfirmation = async (email, done) => {
  ...
  const msg = t('account.confirm_message', { company: COMPANY, baseUrl: FRONTEND_URL, code: token })
  const obj = {
    email,
    token,
    type: FIRST_CONFIRM,
  ...
  const e = await sendEmail(email, subject, msg, 'ConfirmAccount').catch((e) => done(e))
  if (!e) {
    done()
  }
}
...
export const genUser = async (data, final) => {
  ...
  if (valid) {
    ...
    if (u.email && u.password && u.tosAgreement) {
      const exists = await db.read('users', u.email).catch(async (e) => {
...
export const edit = async (data, final) => {
  ...
  const _validUser = await validUser(data.body).catch(async (e) => final(null, { s: 400, e }))
  if (valid && _validUser) {
    await setLocale(data)
    ...
    if (userData.confirmed.email || userData.confirmed.phone) {
      const newData = await editFields(data.body, userData).catch(() => final(null, { s: 400, e: t('error.unknown') }))
      if (newData) {
