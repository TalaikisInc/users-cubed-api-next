...
export default async (data, done) => {
  ...
  if (valid) {
    ...
    const u = await user(data)
      .catch(() => sendError(400, t('error.required'), done))
    if (u.email) {
      const userData = await read('users', u.email)
        .catch(() => done(400, { error: t('error.no_user') }))
      if (userData) {
        await sendReset(u.email, userData.phone)
          .catch(() => sendError(500, t('error.email'), done))
